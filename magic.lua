...
local Window = Rayfield:CreateWindow({
   ...
   KeySettings = {
      Title = "jaja key",
      Subtitle = "Key System",
      Note = "the key is jaja1219 ",
      FileName = "nigger", -- It is recommended to use something unique as other scripts using Rayfield may overwrite your key file
      SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
      GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like Rayfield to get the key from
      Key = {"jaja1219"} -- List of keys that will be accepted by the system, can be RAW file links (pastebin, github etc) or simple strings ("hello","key22")
   }
})
...
local Button = MainTab:CreateButton({
   Name = "aimlock and esp gui",
   Callback = function()
 -- Ong this took me forever to make. Also when enabling the aimbot make sure to make the fov and Click on M2
...
game:GetService("StarterGui"):SetCore("SendNotification", {Title = "Made By Homadb1 On Github", Text = "Enjoy !"})
...
local Button = MainTab:CreateButton({
   Name = "primery aimlock",
   Callback = function()
local OrionLib = loadstring(game:HttpGet(('https://raw.githubusercontent.com/shlexware/Orion/main/source')))()
 
local Window = OrionLib:MakeWindow({Name = "❗South Bronx AUTOFARM❗", HidePremium = false, SaveConfig = true, ConfigFolder = "❗❗❗"})
...
OrionLib:MakeNotification({
	Name = "script creator @veedsi",
	Content = "subscribe2@veedsi",
...
Tab:AddButton({
	Name = "NOCLIP(EXECUTE BEFORE AUTOFARMS!)",
	Callback = function()
      		local player = game.Players.LocalPlayer
...
local function moveToTarget(targetPosition, speed)
...
-- Main loop to move between the two parts and perform actions
local function moveInLoop()
    local part1 = game:GetService("Workspace").trashcan.Union
    local part2 = game:GetService("Workspace").GarbageDumpster
    local targetParts = {part1.Position, part2.Position}
    local currentIndex = 1
    local fastSpeed = 16  -- Set walking speed to 16
    local normalSpeed = 16  -- Optionally set the normal speed to the same value, or adjust as needed
 
    while true do
        local targetPosition = targetParts[currentIndex]
        local speed = (currentIndex == 2) and fastSpeed or normalSpeed
 
        moveToTarget(targetPosition, speed)
 
        if currentIndex == 1 then
            holdEKey(2.5)  -- Hold down 'E' key for 2.5 seconds at part1
        elseif currentIndex == 2 then
            pressNumberKey3()  -- Press number key 3 before starting to move to part2
            wait(0.5)  -- Short wait to ensure key press is registered before moving
            holdEKey(2.5)  -- Hold down 'E' key for 2.5 seconds at part2
        end
 
        currentIndex = currentIndex % #targetParts + 1  -- Toggle between 1 and 2
    end
end
 
-- Run the loop when the character spawns
player.CharacterAdded:Connect(function()
    moveInLoop()
end)
 
-- Run the loop immediately if the character is already present
if player.Character then
    moveInLoop()
end
...
Tab:AddLabel("discord.gg/gFNTebhGFU")
...
Tab:AddButton({
	Name = "esp!",
	Callback = function()
      		-- Services and Local Variables hotkey e
local Players = game:GetService("Players")
local RunService = game:GetService("RunService")
local UserInputService = game:GetService("UserInputService")
local LocalPlayer = Players.LocalPlayer
    ...
    if not Uis:GetFocusedTextBox() then
        if a == getgenv().AimlockKey then
            if AimlockTarget == nil then
                ...
                if Target then AimlockTarget = Target end
                MousePressed = true
            else
                AimlockTarget = nil
                MousePressed = false
